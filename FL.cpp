...
int main(int argc, char** argv){
	...
	ifstream fid_in;
	fid_in.open("/home/yingxiang/Scaling/DT/StayRegionFiltered.txt",ifstream::in);
	FILE * fout_id2;	
	string file_name0="FL"+ thres_min_str + "to" + thres_max_str + ".txt";
	...
	while (getline(fid_in,tline)){
		...
		if (num1==person_id){
			//cout<<10<<" "<<num5<<" "<<personal_record_num<<endl;
			//loc_vec.push_back(num5);
			...
			if (num_place>=threshold_min&&num_place<=threshold_max){
				...
				for (int i=0;i<personal_record_num;i++){
					personal_freuency_count[id_map[loc_vec[i]]]++; // was causing malloc error
				}
